...
void GameManager::advance()
{
    ...
    case GamePhase::FinalExam:
        ...
        addLog(QStringLiteral("进入学期结算。现在可以出售学生转学，或继续下一学期。"));
        emit stateChanged();
        return;
    ...
    case GamePhase::Settlement:
        if (m_semester >= GameConfig::TotalSemesters) {
            ...
            const int den = finalDenominator();
            const double rate = static_cast<double>(rec) / static_cast<double>(den);
            if (rate >= GameConfig::TargetRecommendRate) {
                finishGame(true, QStringLiteral("最终保研率 %1/%2 = %3%，达到目标，胜利！")
                           .arg(rec).arg(den).arg(rate * 100.0, 0, 'f', 1));
            } else {
                finishGame(false, QStringLiteral("最终保研率 %1/%2 = %3%，未达到目标，失败。")
                           .arg(rec).arg(den).arg(rate * 100.0, 0, 'f', 1));
            }
            return;
        }
...
{
    if (!canSell()) {
        emit errorMessage(QStringLiteral("只能在学期结算阶段出售学生。"));
        return;
    }
    if (studentIndex < 0 || studentIndex >= m_students.size()) {
        emit errorMessage(QStringLiteral("请选择有效学生。"));
    ...
    m_coins += income;
    addLog(QStringLiteral("转学出售：%1，获得金币 %2。注意：转学学生不可能成为保研人数。").arg(name).arg(income));
    emit stateChanged();
}
